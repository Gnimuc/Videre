...
# init key signals
const KEYS = (:A, :D, :PAGE_UP, :PAGE_DOWN, :W, :S, :LEFT, :RIGHT)
for key in KEYS
...
function key_callback(window::GLFW.Window, key::Cint, scancode::Cint, action::Cint, mods::Cint)
    ...
    key == GLFW.KEY_PAGE_UP && action == GLFW.PRESS && push!(sigPAGE_UP, true)
    key == GLFW.KEY_PAGE_UP && action == GLFW.RELEASE && push!(sigPAGE_UP, false)
    key == GLFW.KEY_PAGE_DOWN && action == GLFW.PRESS && push!(sigPAGE_DOWN, true)
    key == GLFW.KEY_PAGE_DOWN && action == GLFW.RELEASE && push!(sigPAGE_DOWN, false)
