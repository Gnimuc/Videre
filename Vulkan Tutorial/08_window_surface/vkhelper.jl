...
function find_queue_families(device::VkPhysicalDevice, surface::VkSurfaceKHR)
    ...
    for (i,family) in enumerate(families)
        present_ref = Ref(VkBool32(false))
        vkGetPhysicalDeviceSurfaceSupportKHR(device, i, surface, present_ref)
        if present_ref[] == VkBool32(true)
...
function LibVulkan.VkDeviceCreateInfo(queue_count::Integer, queue_info::Vector{VkDeviceQueueCreateInfo}, features::Ref{VkPhysicalDeviceFeatures})
    sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO
    pNext = C_NULL       # reserved for extension-specific structure
    flags = UInt32(0)    # reserved for future use
    queueCreateInfoCount = queue_count
    pQueueCreateInfos = pointer(queue_info)
    pEnabledFeatures = Base.unsafe_convert(Ptr{VkPhysicalDeviceFeatures}, deviceFeaturesRef)
    return VkDeviceCreateInfo(sType, pNext, flags, queueCreateInfoCount, pQueueCreateInfos, 0, C_NULL, 0, C_NULL, pEnabledFeatures)
end
...
"""
    VkDeviceUniqueQueueFamilyCreateInfos(indices::QueueFamilyIndices, priority::Ref{Cfloat}) -> Vector{VkDeviceQueueCreateInfo}
Return a create infos for the current `QueueFamilyIndices`.
"""
function VkDeviceUniqueQueueCreateInfos(indices::QueueFamilyIndices, priority::Ref{Cfloat})
    ...
    for queueFamily in uniqueQueueFamilies
        ...
        push!(queueCreateInfos, VkDeviceQueueCreateInfo(queueFamilyIndex, 1, queuePriorityRef))
    end
